...
def test_package_add_part_error_if_part_exists_already(package, presentation_part):
    with pytest.raises(ValueError):
        package.add_part(str(presentation_part.uri), None)
...
def test_package_add_part_uri_is_added_to_parts(package):
    part = package.add_part("/ppt/some/part.xml", None)
    assert package.exists_part(str(part.uri))


def test_package_get_part_uri_match(package, presentation_uri_str):
    assert str(package.get_part(presentation_uri_str).uri) == presentation_uri_str
...
def test_package_get_parts_with_rels_type_count(package, presentation_relspart):
...
def test_package_remove_part(package, presentation_part):
    ...
    package.remove_part(str(presentation_part.uri))
    assert not package.exists_part(str(presentation_part.uri))
    assert package.get_parts(presentation_part.type) == []


def test_package_remove_part_but_types_still_has_it(package, presentation_relspart):
    before_count = len(package.get_parts(presentation_relspart.type))
    package.remove_part(str(presentation_relspart.uri))
