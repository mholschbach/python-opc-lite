...
def test_uri_str_value(get_uri_presentation):
    assert get_uri_presentation.str == '/ppt/presentation.xml'
...
@pytest.mark.parametrize("uri_str,expectation", [('/_rels/.rels', True),('/docProps/core.xml', False)])
def test_uri_is_rels(uri_str,expectation):
    assert Uri(uri_str).is_rels == expectation

@pytest.mark.parametrize("uri_str, expectation", [('/', '/_rels/.rels'),('/docProps/core.xml', '/docProps/_rels/core.xml.rels')])
def test_uri_rels(uri_str, expectation):
    assert Uri(uri_str).rels == expectation

@pytest.mark.parametrize("zipname, expectation", [('_rels/.rels','/_rels/.rels'),('docProps/core.xml', '/docProps/core.xml')])
def test_uri_zipname2str(zipname, expectation):
    assert Uri.zipname2str(zipname) == expectation

@pytest.mark.parametrize("uri_str, rel_target_uri_str, expectation", [
    ...
    ('/ppt/slides/slide1.xml', "../embeddings/Microsoft_Excel_Worksheet.xlsx", "/ppt/embeddings/Microsoft_Excel_Worksheet.xlsx"),
    ('/ppt/handoutMasters/handoutMaster1.xml', "../theme/theme4.xml", "/ppt/theme/theme4.xml"),
    ])
